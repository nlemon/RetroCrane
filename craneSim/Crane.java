...
public class Crane implements Displayable{
	...
	class Arm extends DisplayableObject{
	    ...
	    Arm(double angl, Point2D anch, Color c, int w, int h){			
	...
	public void initializeArms(){
		...
		Point2D armAnchor = new Point2D.Double(5, 10);
		
		arm1 = new Arm(-Math.PI/4.0, armAnchor, Color.RED, 70, 20);
		arm2 = new Arm(0.3, armAnchor, Color.RED, 70, 20);
		arm3 = new Arm(0.3, armAnchor, Color.RED, 70, 20);
		arm4 = new Arm(0.3, armAnchor, Color.RED, 70, 20);
		magnet = new Magnet(Color.GREEN);

		Point2D arm1Position = new Point2D.Double(13, -5);
		Point2D armPosition = new Point2D.Double(arm1.getWidth() - 5, 0);
		...
		magnet.getMyTransform().translate(70, -5);
	}
	...
	public void initializeBody(){
		...
		tracks = new Body(160, 30, Color.BLUE);
	...
	if((magnetBase <= (ground + 5)) && (magnetBase >= (ground - 5))){
		...
		if(blokes.get(i).getFrame().intersects(boundingBox)){
		    success = false; break;
		}
		success = true;
	    }		
	...
	for(int i = 0; i < blokes.size(); i++){
	    ...
	    double lower = bound.getY() - bound.getHeight();
  
	    if(((Math.abs(maggie.getWidth() - bound.getX()) <= 4) && (Math.abs(maggie.getWidth() - bound.getX()) <= 4)) &&
	       (Math.abs(0 - bound.getY()) < (1.5 * maggie.getHeight())) && (lower <= maggie.getHeight())){//magnet and block are reasonably close

		    /* add a new crane component that mimics the block in dimension */
		Arm magnetBlock = new Arm(0.0, new Point2D.Double(), Color.RED, blokes.get(i).getHeight(), blokes.get(i).getWidth());
		    magnetBlock.getMyTransform().translate(maggie.getWidth(), bound.getY());
		    magnetBlock.setMagnet(true);
		    magnetBlock.setBlock();
